...
- 入站 HTTP 签名只接受由活动 Actor 本身提供、ID 与 owner 均匹配的公钥，防止远端自报密钥冒充其他账号。
- 文章 Delete 活动改为 Typecho 确认删除成功后再投递，避免权限检查失败时误删远端文章。
- 远端回复与本地评论的关联改为单独持久化；清理旧活动前迁移尚存的历史关联，保留已删除回复的墓碑。
- 保留仍有效与已撤销的点赞、转发状态，不再因活动日志过期丢失统计或重新接受旧互动。
- 签名 POST 支持安全 HTTPS 301、302、307、308 重定向，并针对目标 URL 重新签名；发件箱增加历史文章分页。

## 0.6.0

- 联邦回复默认写入待审核状态，并在新建和远端编辑时接入 Typecho 原生评论过滤器。
- 新增按 Actor 和来源域统计的 5 分钟频率限制与保留期存储配额，超限请求返回 `429`。
- 新增带索引的入站额度台账，随过期活动日志一起清理。
- 新增位于“设置”之后的顶级“联邦宇宙”菜单，将时间轴、关注者和正在关注拆分为三个独立子页面。
