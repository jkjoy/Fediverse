...
- 每位作者可设置联邦头像、Banner 和 4 组 Mastodon 兼容资料字段
- 作者保存资料后向关注者发送 `Update Person`，促使远端实例刷新资料缓存
...
4. 每位作者在个人设置中的插件配置里确认联邦用户名、简介、头像、Banner 和资料字段。
...
从 `0.4.3` 升级到 `0.5.0` 无需迁移数据表。作者可直接在个人设置的 Fediverse 配置中填写头像、Banner 和最多 4 组资料字段，保存后远端再次获取 Actor 即可看到更新。

从 `0.5.0` 升级到 `0.5.1` 无需迁移数据表。文章互动区的联邦回复数量现在包含已接收但仍在等待审核的回复；回复正文仍需在 Typecho 评论后台审核通过后才会公开显示。
...
后台支持立即处理当前批次、重新投递失败任务、删除无效任务、移除关注者、批量生成作者身份、同步作者资料、补发最近 20 篇公开文章和清理过期活动。联邦地址及端点可直接复制，操作完成后会保留当前视图。所有管理操作均要求管理员权限并通过 Typecho 安全令牌校验。
...
插件不会全局改写文章正文。主题需要在文章详情模板中调用渲染函数，才能显示联邦点赞、转发、回复数量和最近互动者。当前站点的 Berry 主题包含以下两处修改；更新或更换主题后需要重新应用。
...
- `replies`：本站已接收且尚未删除或撤销的联邦回复数量。
- `recent`：最多 12 条最近点赞或转发记录，每项包含 `type`、`actor` 和 `created`。

已忽略、垃圾、已删除或已撤销的活动不会计入公开统计。回复正文由 Typecho 原生评论列表输出，互动区不会重复渲染评论正文。
...
- 头像和 Banner 必须使用可公开访问的 HTTPS 图片地址；插件只发布地址，不下载或代理图片。
- 最多 4 组资料字段，每组名称和内容都非空时才会发布；内容是 HTTPS 地址时输出为 `rel=me` 链接，其他内容按纯文本转义，不能直接插入 HTML。
- 作者首次被发现或首次发布时生成 2048 位 RSA 密钥。
- 修改已联邦化的用户名会改变 Actor URL，不建议修改。

## 数据与评论

远端 `Note` 回复仅在 `inReplyTo` 指向本站已发布文章、文章允许评论且收件人匹配作者时接收。内容转成纯文本，并以 `approved` 状态写入 Typecho 评论表直接显示。评论作者优先使用远端 Actor 的显示昵称，资料不可用时才回退到用户名或账号标识。
...
5. 从远端回复，确认 Typecho 后台出现已通过评论，作者显示远端昵称。
