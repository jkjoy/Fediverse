...
当前版本：`0.6.1`。完整版本记录见 [CHANGELOG.md](CHANGELOG.md)。
...
- 投递队列支持指数退避、每批处理上限和最多 8 次重试；收件箱 HTTPS 重定向时重新签名。
- 提供按 Actor、来源域的入站速率限制和存储配额。
- 非持久状态的入站活动、时间轴和额度台账按保留期清理；有效互动与回复对象关联独立保留。
...
Cron 会处理当前批次的投递任务，并清理超过保留期的普通入站活动、时间轴和额度台账。仍有效或已撤销的点赞、转发记录及远端回复对象关联会继续保留，以维持计数、去重与远端编辑或删除。任务失败后按指数退避重试；连续失败 8 次后停止自动重试并保留在后台。
...
| 来源版本 | 升级操作 |
| --- | --- |
| `0.6.0` | 完整覆盖后先打开一次“管理 → 联邦管理”创建远端回复关联表，再运行 Cron 或接收新活动；首次清理前会将仍在日志中的旧回复关联迁入新表 |
...
升级到 `0.6.1` 后须先打开插件后台完成建表。已在旧版本清理掉的回复活动无法从日志恢复关联；这些旧评论可保留在 Typecho 中，但之后的远端编辑或删除无法自动匹配。尚存的旧日志会在下次清理时迁移。新增发件箱 `?page=1`、`?page=2` 等页面，可供远端读取较早文章。
...
- Activity ID 用于去重；入站签名公钥必须出现在对应 Actor 文档中，且密钥 ID 与 owner 都必须匹配。
