...
- Mastodon、Misskey 等远端回复经 Typecho 评论过滤器处理后写入待审核评论，并同步处理远端编辑与删除
- HTTP Signatures、Digest、Date 验证和活动去重
- 后台投递队列、指数退避和最多 8 次重试
- “管理 → 联邦管理”提供作者账号、投递队列和入站活动
- “联邦宇宙”顶级菜单提供时间轴、关注者和正在关注三个独立页面
...
从 `0.3.0` 升级到 `0.4.0` 后，打开一次“管理 → 联邦管理”，插件会自动创建关注、时间轴和本地回复数据表，并注册回复对象路由，无需停用插件。
...
从 `0.5.2` 升级到 `0.5.3` 无需迁移数据表。作者保存个人 Fediverse 设置后会向关注者发送资料更新通知；文章发布会按数据库中的最终状态可靠入队。升级后可在“管理 → 联邦管理”使用“同步作者资料”和“补发最近文章”修复远端缓存及历史漏投，然后运行投递队列。

从 `0.5.3` 升级到 `0.5.4` 无需迁移数据表。文章互动区改为不依赖具体主题变量的通用样式，并补充移动端、深色、高对比度、键盘焦点和减少动态效果支持；主题调用方式不变。

升级到当前开发版本后，先打开一次原有的“管理 → 联邦宇宙”（加载后会改名为“联邦管理”），以创建入站额度台账并注册新的顶级“联邦宇宙”菜单。刷新后台后即可看到“时间轴、关注者、正在关注”三个子项。新接收和远端编辑的回复会进入待审核状态，并经过 Typecho 的 `Widget_Feedback:comment` 过滤器；已有评论状态不会自动改变。
...
“管理 → 联邦管理”支持立即处理当前批次、重新投递失败任务、删除无效任务、批量生成作者身份、同步作者资料、补发最近 20 篇公开文章和清理过期活动。顶级“联邦宇宙”菜单分别提供时间轴、关注者和正在关注页面，操作完成后会返回对应页面。联邦地址及端点可直接复制，所有管理操作均要求管理员权限并通过 Typecho 安全令牌校验。

## 关注与时间轴

进入“联邦宇宙 → 正在关注”，选择一个本地作者身份并输入完整账号，例如 `@user@example.com`。插件会通过 WebFinger 查找远端 Actor，并把签名 `Follow` 加入投递队列。远端返回 `Accept` 后状态变为“已关注”；返回 `Reject` 时显示“已拒绝”。
...
远端 `Note` 回复仅在 `inReplyTo` 指向本站已发布文章、文章允许评论且收件人匹配作者时接收。内容转成纯文本，经过 Typecho 评论过滤器后以 `waiting` 状态写入评论表；过滤器仍可将其标记为垃圾或直接拒绝。评论作者优先使用远端 Actor 的显示昵称，资料不可用时才回退到用户名或账号标识。

插件默认限制单个 Actor 每 5 分钟 30 条活动、单个来源域每 5 分钟 100 条活动；尚未过期的入站台账默认最多保留单个 Actor 2,000 条、单个来源域 10,000 条。四项限制可在插件设置中调整，填写 `0` 可停用对应限制，Cron 清理过期活动时会同步释放存储额度。
...
5. 从远端回复，确认 Typecho 后台出现待审核评论，作者显示远端昵称，审核通过后才在文章页显示。
6. 在“管理 → 联邦管理 → 投递队列”确认没有持续失败任务。
